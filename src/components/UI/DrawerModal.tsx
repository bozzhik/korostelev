...
export function Modal({type, tag, heading, content, achievements, image, source, children}: ModalProps) {
  ...
  return (
    <Drawer>
      ...
      <DrawerContent className={`p-20 py-14 xl:p-10 sm:p-5 flex flex-col mx-auto w-[62%] ${isTeam ? 'xl:w-[60%]' : 'xl:w-[80%]'} sm:w-[93%] text-background bg-red`}>
        ...
        <DrawerHeader className={`${isTeam || isProject ? 'space-y-40 max-h-[80vh] sm:max-h-[90vh]' : 'space-y-10 max-h-[90vh]'} ${isProject && 'xl:space-y-16'} w-full sm:space-y-5`}>
          <div className="space-y-3 max-w-[60%] sm:max-w-none">
            ...
            <div className={`${isTeam ? 'w-[30%] xl:w-[35%]' : 'w-[35%]'} sm:hidden !m-0 absolute top-0 right-0 -z-20 flex flex-col`}>
              {image && <Image quality={100} className={`object-cover ${isTeam ? 'max-h-[32vh]' : 'max-h-[27vh]'}`} width={500} height={500} src={image} alt="" />} {/* main image */}
            </div>
          </div>
