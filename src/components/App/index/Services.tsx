...
const servicesData: Record<string, Service> = {
  ...
  8: {
    ...
    content: [
      `Опыт юристов Korostelev & Partners, полученный, в том числе, в Федеральном агентстве по недропользованию (Роснедра) – государственном органе, выдающем лицензии на право пользования недрами, позволяет консультировать российских и иностранных клиентов по любым юридическим вопросам, возникающим на различных этапах жизненного цикла проектов по добыче нефти, газа и твердых полезных ископаемых – от геологического изучения, разведки и разработки месторождений до реализации продукции.`,
      ...
      <br/>
        •   сопровождение сделок M&A в сфере недропользования; сопровождение создания совместных предприятий в сфере недропользования
      <br/>
...
export default function Services({where}: {where: 'index' | 'services'}) {
  ...
  const isServices = where === 'services'
  ...
  return (
    <section id={!isServices ? 'services' : ''} data-section="services-index" className="relative z-20">
