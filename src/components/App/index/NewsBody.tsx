  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 sm:hidden">
        <div className="flex flex-col justify-between">
          {newsData.map(({heading, tag, date, content, image, source}, index) => (
            <ModalTrigger key={index} tag={`${date} // ${tag}`} heading={heading} content={content} image={urlForImage(image)} source={source}>
              <div onMouseEnter={() => handleNewsHover({heading, tag, date, content, image, source})} className="relative h-full pl-10 text-left duration-200 cursor-pointer py-7 group hover:bg-red hover:text-background hover:pl-14">
        ...
        <Image quality={100} priority={true} className="object-cover w-full h-full" src={urlForImage(selectedNews.image)} alt={selectedNews.heading} />
      </div>
      ...
      <div className="flex-col justify-between hidden gap-0 sm:flex">
        {newsData.map(({heading, tag, date, content, image, source}, index) => (
          <div key={index} onMouseEnter={() => handleNewsHover({heading, tag, date, content, image, source})} className="relative h-full pb-5 space-y-5 duration-200 group hover:bg-red hover:text-background">
            <Image quality={100} className="object-cover aspect-square" src={urlForImage(image)} alt="" />
